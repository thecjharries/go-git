...
func (s *UpdReqDecodeSuite) TestOneUpdateCommand(c *C) {
	...
	name := plumbing.NewReferenceName("myref")
...
func (s *UpdReqDecodeSuite) TestMultipleCommands(c *C) {
	...
	expected := NewReferenceUpdateRequest()
	expected.Commands = []*Command{
		{Name: plumbing.NewReferenceName("myref1"), Old: hash1, New: hash2},
		{Name: plumbing.NewReferenceName("myref2"), Old: plumbing.ZeroHash, New: hash2},
		{Name: plumbing.NewReferenceName("myref3"), Old: hash1, New: plumbing.ZeroHash},
	}
	expected.Packfile = ioutil.NopCloser(bytes.NewReader([]byte{}))
